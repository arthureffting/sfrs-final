...
parser.add_argument("--testing_images_per_epoch", default=40)
...
# Training techniques
parser.add_argument("--name", default="training-orcas")
parser.add_argument("--reset-threshold", default=16)
...
training_set_list_path = os.path.join("data", "orcas", "prepared", "pages", "training.json")
...
test_set_list_path = os.path.join("data", "orcas", "prepared", "pages", "testing.json")
...
validation_path = os.path.join("data", "orcas", "prepared", "pages", "validation.json")
validation_list = load_file_list_direct(validation_path)
validation_set = LolDataset(validation_list[3:4])
...
for epoch in range(1000):
    ...
    for index, x in enumerate(test_dataloader):
        ...
        if baseline_steps is None or len(baseline_steps) == 0:
            continue
