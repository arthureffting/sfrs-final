...
class LineOutlinerTsa(nn.Module):
    ...
    def __init__(self, path=None, patch_ratio=5, tsa_size=3, min_height=32, patch_size=64):
        ...
        self.fully_connected = nn.Linear(512, 6)
        #self.fully_connected.bias.data[0] = 0  # Base x
        #self.fully_connected.bias.data[1] = 0  # Base y
        #self.fully_connected.bias.data[2] = 0  # Next angle
        #self.fully_connected.bias.data[3] = 0  # Upper height %
        #self.fully_connected.bias.data[4] = -5  # Lower height % bias is 0
        #self.fully_connected.bias.data[5] = -5  # Stop confidence bias is 0
    ...
    def forward(self,
        ...
        current_height = Variable(torch.dist(sol["upper_point"].clone().cuda(), sol["base_point"].clone().cuda()),
                                  requires_grad=True)
        if disturb_sol:
            current_height *= random.uniform(-height_disturbance, height_disturbance)

        current_scale = (self.patch_ratio * current_height / self.patch_size).cuda()
        current_angle = Variable(sol["angle"].clone().cuda(), requires_grad=True)
        ...
        while max_steps is None or steps_ran < max_steps:
            ...
            patch_parameters = torch.stack([current_base[0],  # x
                                            ...
                                            torch.dist(sol["upper_point"].clone(),
                                                       sol["base_point"].clone()).cuda()]).unsqueeze(0)
            ...
            if min_steps is not None and steps_ran < min_steps:
                continue
